...
#CRIAR PUBLICAÇÃO
def criar_autores():
    resp = str(input("Deseja adicionar autores? [s/n]").strip().lower())
    if resp == "s":
        authors = []
        X = True
        while X:
            ...
            print("Deseja adicionar mais autores? [s/n]")
...
def abrir_janela_pesquisar_autores(data):
    ...
    authors = []
    articles = []
    list_box_autores = sg.Listbox(authors, size=(70, 30), font=font, key="-AUTHOR-", enable_events=True)
    list_box_articles = sg.Listbox(articles, size=(70, 32), font=font, key="-ARTICLE-", enable_events=True)
    ...
    window = sg.Window('Pesquisar Autores', column1_layout, modal=True, location=(0,0))
    ...
    while not stop:
        ...
        if event == sg.WINDOW_CLOSED or event == 'Close':
            ...
            if authors != []:
                ...
                sg.popup("Selecione a ordem que pretende visualizar os autores.")
        elif event == "-ARTICLE-":
            if articles != []:
                article = values["-ARTICLE-"][0]
                janeladetalhes(article, data)
            else:
                sg.popup("Selecione um autor para visualizar os artigos.")
    window.close()
...
def abrir_janela_ajuda():
    ...
    - Export: Permite exportar para ficheiro os registos resultantes de
                 uma pesquisa (apenas o subconjunto retornado pela
                 pesquisa).
    ...
    layout = [
        ...
        [sg.Button("Fechar", button_color=("#044343", "pink"))]
    ]
